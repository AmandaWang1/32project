...
void ILI9341_DispChar_EN ( uint16_t usX, uint16_t usY, const char cChar )
{
	...
	ILI9341_Write_Cmd ( CMD_SetPixel );	//准备传输数据给液晶屏

	//按字节读取字模数据
	//由于前面直接设置了显示窗口，显示数据会自动换行
	//byteCount用来记录字节数，bitCount用来记录位数
...
void ILI9341_DispString_EN ( 	uint16_t usX ,uint16_t usY,  char * pStr )
{
	while ( * pStr != '\0' )//判断等于字符串结束时结束循环31
	{
		...
		ILI9341_DispChar_EN ( usX, usY, * pStr);//输入x,y坐标，处理指针中的
...
/**
  ...
  * @retval None
  */
void LCD_SetFont(sFONT *fonts)//选择参数位fonts.c中的结构体参数（此文件最末尾设置字体的）
{
