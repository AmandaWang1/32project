...
static void ADCx_Mode_Config(void)
{
	...
	DMA_InitStructure.DMA_BufferSize = NOFCHANEL;		//改																		// 缓冲区大小为1，缓冲区的大小应该等于存储器的大小
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;						// 外设寄存器只有一个，地址不用递增
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;//改; 										// 存储器地址固定
	...
	ADC_InitStructure.ADC_ScanConvMode = ENABLE;//改									 // 扫描模式，多通道需要，单通道不需要
	ADC_InitStructure.ADC_ContinuousConvMode = ENABLE;								 // 连续转换模式
	ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_None;// 软件触发，不使用外部触发，外部触发为定时器，GPIO
	ADC_InitStructure.ADC_DataAlign = ADC_DataAlign_Right;             // 转换结果右对齐
	ADC_InitStructure.ADC_NbrOfChannel = NOFCHANEL;										 // 要转换的通道数目 转换通道1个
	ADC_Init(ADC_x, &ADC_InitStructure);													     // 初始化ADC
	
	RCC_ADCCLKConfig(RCC_PCLK2_Div8); 																 //9M
	// 配置ADC 通道的转换顺序和采样时间
	ADC_RegularChannelConfig(ADC_x, ADC_CHANNEL1, 1, ADC_SampleTime_55Cycles5);//顺序可以改
