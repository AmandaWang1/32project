...
static void FIFO_GPIO_Config(void)
{
    ...
    /*(FIFO_DATA--FIFO输出数据)*///注意：这里要配置成输入
    ...
    FIFO_OE_L();	  					/*OE引脚低电平 使FIFO输出使能*/
    FIFO_WE_H();   						/*WE拉高 使FIFO写允许*/
...
ErrorStatus OV7725_Init(void)
{
	...
	if( 0 == SCCB_ReadByte( &Sensor_IDCode, 1, 0x0b ) )	 /* 读取sensor ID号,OV7725的ID*/
	{
		//DEBUG("read id faild");		
		return ERROR;
	}
	...
	{
		/*for循环，循环很多次
		每循环一次，调用一次WriteByte函数，这个函数第一个参数是地址，第二个参数是数据*/
		for( i = 0 ; i < OV7725_REG_NUM ; i++ )
		{
			/*下面一条语句的意思是，结构体里有两个成员，一个是Address，一个是Value
			即前一个变量存储了寄存器的地址，后一个变量存储量要写入的内容
			调用这个循环把数组写进去，其实是把一系列参数写入到ov7725芯片中*/
...
/**
  ...
  * @retval 、=-
  */
//可以先认为参数中前两个是0 下一个是320，再下一个是240
//读取像素点
void ImagDisp(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height)
{
	...
	for(i = 0; i < width; i++)
	{
		for(j = 0; j < height; j++)
		{
			...
			ILI9341_Write_Data(Camera_Data);//把像素点放到液晶屏上显示，传一个像素点，指针加一
		}
	}
}
