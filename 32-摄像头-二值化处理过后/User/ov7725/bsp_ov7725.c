...
//volatile 
	uint16_t Ov7725_vsync;	 /* 帧同步信号标志，在中断函数和main函数里面使用 */
...
void ImagDisp(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height)
{
	...
	for(i = 0; i < cam_mode.cam_width; i++)
	{
		...
		{
			READ_FIFO_PIXEL(Camera_Data[i][j]);		/* 从FIFO读出一个rgb565像素到Camera_Data变量 */
//			delay_us(1);
//			ILI9341_Write_Data(Camera_Data[i][j]);
//			READ_FIFO_PIXEL(cam);		/* 从FIFO读出一个rgb565像素到Camera_Data变量 */
////			delay_us(1);
//			ILI9341_Write_Data(cam);
//			
